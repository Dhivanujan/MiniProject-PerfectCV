...
app.config["MONGO_URI"] = "Mongodb Atlas connection string"  # Replace with your real connection string
mongo = PyMongo(app)
bcrypt = Bcrypt(app)
from flask import Flask, render_template, url_for, redirect, flash, request, send_file
from flask_pymongo import PyMongo
...
def clean_text(text):
    """Fallback for unsupported Unicode characters if font is missing"""
    return text.replace("⚠️", "[ERROR]").encode("latin-1", "replace").decode("latin-1")
...
def generate_pdf(text):
    ...
    if os.path.exists(font_path):
        ...
        # Fallback: use Helvetica and clean Unicode
        text = clean_text(text)
        pdf.set_font("Helvetica", size=12)
